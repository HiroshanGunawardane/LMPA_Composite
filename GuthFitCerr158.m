...
try
    ...
    error('Failed to read Cerr 158 file: %s', ME.message);
end

% --- Filter for Cerr 158 ---
data.Material = categorical(data.Material);
data = data(data.Material == 'cerr_158' | data.Material == 'Cerr 158', :);
...
A_fit = paramFit(1);
B_fit = paramFit(2);
fprintf('Optimized Guth parameters for Cerr 158:\n');
fprintf('  A = %.4f\n', A_fit);
fprintf('  B = %.4f\n', B_fit);
...
hold on;
plot(numericConc, E_exp_mpa, 'go-', 'LineWidth', 2, 'DisplayName', 'Experimental Cerr 158');
...
title('Fitting Guth Model to Cerr 158 Data');
